<?php

include_once 'admin/membership-options-map.php';

require_once 'const.php';
require_once 'helper.php';

//Login functions
require_once 'login/load.php';

//Dashboard functions
require_once 'dashboard/load.php';

//Widgets
require_once 'widgets/load.php';

//Shortcodes
require_once 'lib/shortcode-interface.php';
require_once 'shortcodes/shortcodes-functions.php';

//Favorites
require_once 'modules/favorites/load.php';

//load custom styles
if(!function_exists('qode_membership_load_custom_styles')) {
	function qode_membership_load_custom_styles() {
		require_once 'assets/custom-styles/membership.php';
	}
	add_action('after_setup_theme','qode_membership_load_custom_styles');
}